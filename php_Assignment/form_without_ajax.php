...
<body>
  ...
  <a href="form task.php"><button  id="creat_btn1"   type="button"> Creat New User </button></a>
     ...
     <!-- the modal form start -->
     <div id="contact"><button type="button" id="creat_btn2"  class="btn btn-info btn" data-toggle="modal" data-target="#contact-modal">Show Contact Form</button></div>
...
<!-- modal form end -->
    ...
    while ($row = mysqli_fetch_assoc($fetch))
    {
        ...
        <td> $btn</td>
        <td> $row[user_type]</td> 
        <td>  <button id='up_btn'><a href='#' data-id='$row[id]' class= 'text-decoration-none text-white' > Update </a></button>  &nbsp 
       <form action='delete.php' method='POST'>
        <input type='hidden' name='id' id='id' value='$row[id]'>
        
    <div class='right gap-items-2'>
        <button class='btn btn-error submitForm' name='archive' type='submit' >archive</button>
    </div>
</form>
        </td>
        </tr>\n";
